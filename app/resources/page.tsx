...
import { getResourceDetails, isSchoolResource } from 'lib/school-resources';
...
export const metadata: Metadata = {
  ...
  description,
  alternates: { canonical: 'https://schoolkits.org/resources' },
  openGraph: {
    type: 'website',
    title,
    description,
    url: 'https://schoolkits.org/resources',
    siteName: 'SchoolKits',
    images: [
      {
...
export default async function ResourcesPage() {
  ...
  const posts = (await getBlogPosts(250, 'news'))
    .filter(isSchoolResource)
    ...
    <div className="bg-white text-[#073B4C]">
      ...
      >
        <div className="mb-7 flex flex-wrap items-end justify-between gap-4">
          ...
          <Link href="/blog" className="py-2 text-sm font-semibold text-[#0B779A] hover:underline">
            View all blog articles <span aria-hidden="true">→</span>
          </Link>
        </div>
        {posts.length ? (
